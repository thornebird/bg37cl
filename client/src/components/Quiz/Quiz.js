...
const Quiz  = props =>{
 ...
 return (
    ...
    { !completed ? <Row className={styles.background}>
         <Col sm={12} md={12}>
           <span className={styles.title}>Question {count+1}/{props.questions.length}: </span> 
     ...
     <Row className={styles.center}  >
        <Col sm={12} md={12}>
           <span className={styles.title}>We are checking your answers</span>    
          {
            isAnswered ?<p>
             {props.questions.length} out of {props.questions.length} questions answered...
           </p> :''
          }
          {
            isIp ?<p>
             Ip {props.ip} is eligible...
           </p> :''
          } 
          {
            loadingPrize ? <p>
