...
const Quiz  = props =>{
 ...
 return (
    ...
    { !completed ? <Row className={styles.background}>
         <Col sm={12} md={12}>
           <span className={styles.title}>Pregunta {count+1}/{props.questions.length}: </span> 
     ...
     <Row className={styles.center}  >
        <Col sm={12} md={12}>
           <span className={styles.title}>Estamos revisando sus respuestas.</span>    
          {
            isAnswered ?<p>
             {props.questions.length} de {props.questions.length} preguntas respondidas ...
           </p> :''
          }
          {
            isIp ?<p>
             Ip {props.ip} es elegible ...
           </p> :''
          } 
          {
            loadingPrize ? <p>
